...
export function ReisAanmaken({
  ...
  // Eén bron voor beide manieren van tonen: de markering en het lijstje.
  const verplicht = {
  ...
  return (
    <div className="space-y-4">
      ...
      <NogInvullen velden={ontbreekt} />

      <div className="space-y-4 lg:grid lg:grid-cols-2 lg:items-start lg:gap-4 lg:space-y-0">
        <div className="space-y-4">
          <Kaart className="space-y-3">
            <Veld label="Naam van de reis" verplicht ingevuld={verplicht["Naam van de reis"]}>
              <input
            ...
            <div className="grid grid-cols-2 gap-3">
              <Veld label="Vertrek" verplicht ingevuld={verplicht["Vertrekdatum"]}>
                <input
                  type="date"
                  className={INVOER_STIJL}
                  value={vertrekdatum}
              ...
              <Veld label="Terug" verplicht ingevuld={verplicht["Terugdatum"]}>
                <input
                  type="date"
                  className={INVOER_STIJL}
          ...
          <Kaart className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Veld
                label="Thuisplaats"
                verplicht
                ingevuld={verplicht["Thuisplaats"]}
                hint="Vertrekpunt van de route."
              >
                <input
                  className={INVOER_STIJL}
                  placeholder="Utrecht"
                ...
                <Veld label={`Reiziger ${index + 1}`}>
                  <input
                    ...
                    onChange={(event) =>
                      setReizigers(
                        reizigers.map((r, i) =>
                          i === index ? { ...r, naam: event.target.value } : r,
                        ),
                      )
                    }
                  />
                      ...
                      setReizigers(
                        reizigers.map((r, i) =>
                          i === index
                            ? {
                                ...r,
                                geboortejaar: event.target.value.replace(/\D/g, "").slice(0, 4),
                              }
                            : r,
                        ),
                      )
